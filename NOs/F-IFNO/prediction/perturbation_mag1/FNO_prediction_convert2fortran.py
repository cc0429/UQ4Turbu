...
vor_data_perturbation1_ft = (1+1) * vor_data_perturbation1_ft # magnititude perturbation of all modes
...
#cases = [numpy_pre_vor_t_total[..., i] for i in range(30)]
...
for q in range(len(casenumber)):
    tn = qnumber[q]
    print(casenumber[q])
    f = open("./600/FIFNO32_uxyz_gap200_case{}.dat".format(tn), "w")
    for n in range(eval(casenumber[q]).shape[0]):
        for m in range(eval(casenumber[q]).shape[1]):
            for i in range(eval(casenumber[q]).shape[2]):
                for j in range(eval(casenumber[q]).shape[3]):
