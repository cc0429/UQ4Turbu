...
vor_data_perturbation1 = vor_data[0:30,0,:,:,:,:]
#vor_data_perturbation2 = vor_data[15:30,0,:,:,:,:]
...
vor_data_perturbation1_ft = (1+magnititude_perturbation) * vor_data_perturbation1_ft # magnititude perturbation of all modes
#vor_data_perturbation2_ft = (1+0.1) * vor_data_perturbation2_ft # magnititude perturbation of all modes
...
vor_data[0:30,0,:,:,:,:] = vor_data_perturbation1
#vor_data[15:30,300,:,:,:,:] = vor_data_perturbation2
#######################################加载训练好的模型
...
for sample_id in sample_id_data:
    for time_id in time_id_data:    #起始步
        ...
        for i in range(time_advanced_step):  
            ...
            #print("check for shape", predict_vor.shape)
            #print(np.iscomplexobj(predict_vor))
            # 设定网格大小
            Nx, Ny, Nz = 32, 32, 32
            ...
            #print("调整后的速度场形状:", predict_vor.shape)
...
for q in range(len(casenumber)):
    tn = qnumber[q]
    print(casenumber[q])
    f = open("./600/m{}/F-IFNO32_uxyz_gap200_case{}.dat".format(magnititude_perturbation,tn), "w")
    for n in range(eval(casenumber[q]).shape[0]):
        for m in range(eval(casenumber[q]).shape[1]):
            for i in range(eval(casenumber[q]).shape[2]):
                for j in range(eval(casenumber[q]).shape[3]):
