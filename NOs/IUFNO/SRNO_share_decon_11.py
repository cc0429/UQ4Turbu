...
for ep in range(epochs):
    ...
    if ep == 0:
        print("Epoch,","Time,","[Train Recons MSE],","[Train Pred MSE],","[Test Recons MSE],","[Test Pred MSE]")
    print(ep, "%.2f"%(t2-t1), "%.6f"%(train_recons_full), "%.6f"%(train_pred_full), 
          "%.6f"%(test_recons_full), "%.6f"%(test_pred_full))
    torch.save(model.state_dict(), f'm12w90n40g45_{ep+1}ep1e8w.pth')  # 注意修改保存模型格式
    
MSE_save = np.dstack((train_pre, train_recon,test_pre,test_recon)).squeeze()
np.savetxt('./loss_data.dat', MSE_save, fmt="%16.7f")
