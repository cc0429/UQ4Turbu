...
for kk, case_number in enumerate(case_number_list):
    ...
    xlimit = [1, 10]
    #ylimit = [-1.6, 1.6]
    legend1 = ["$\\tilde{\\varepsilon}$=0", "$\\tilde{\\varepsilon}$=0.1","$\\tilde{\\varepsilon}$=0.5", "$\\tilde{\\varepsilon}$=1","$\\tilde{\\varepsilon}$=2", "$\\tilde{\\varepsilon}$=5","$\\tilde{\\varepsilon}$=10","fDNS"]
    ...
    mpl.rc('font', family='STIXGeneral')
    # 创建图例
    lgd = plt.legend(loc='upper right', fontsize=60, ncol=2,  frameon=True, framealpha=0, edgecolor='black', shadow=False,columnspacing=0)
    #plt.title("spectrum errorbar".format(vel_k), fontsize=70, color='black', loc='center', pad=15)
    plt.ylim(-0.014, 0.064) 
    # 显示网格
    #plt.grid()
    figPath1 = os.path.abspath("./vel_k_errorbar_with_mag_with_k/{}cases".format(case_number))
    gfile1 = "spectrum errorbar for F-IFNO_1.png"
