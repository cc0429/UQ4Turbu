...
for k, case_number in enumerate(case_number_list):
    ...
    yanse=colors[1]
    plt.plot(x1, mean2, color=yanse, alpha=0.8, linewidth=2*lineWidth, linestyle='--',zorder=2)
    ...
    yanse=colors[2]
    plt.plot(x1, mean3, color=yanse, alpha=0.8, linewidth=2*lineWidth, linestyle='--',zorder=2)
    ...
    yanse=colors[3]
    plt.plot(x1, mean4, color=yanse, alpha=0.8, linewidth=2*lineWidth, linestyle='--',zorder=2)
    ...
    yanse=colors[4]
    plt.plot(x1, mean5, color=yanse, alpha=0.8, linewidth=2*lineWidth, linestyle='--',zorder=2)
    ...
    yanse=colors[5]
    plt.plot(x1, mean6, color=yanse, alpha=0.8, linewidth=2*lineWidth, linestyle='--',zorder=2)
    ...
    lgd = plt.legend(
        loc='upper left',  bbox_to_anchor=(0.0, 1.0),  ncol=2,      # 自动选择最优位置
        fontsize=30,
        frameon=True,
        framealpha=0,
        edgecolor='black',
    ...
    #plt.grid()
    figPath1 = os.path.abspath("./vel_k_errorbar_with_k/{}cases".format(case_number))
    gfile1 = "Errorbar for E(k=1-10) IUFNO.png"
    gpath1 = os.path.join(figPath1, gfile1)
    # 保存图形
    plt.savefig(gpath1, bbox_inches='tight')
    plt.close()
    ...
    xlabPad, ylabPad = 10, 10
    xlabel = r"$\mathdefault{k}$"
    ylabel = r"$\mathdefault{Fluctuation}$"
    xlimit=[0,0.42]
    #ylimit = [-0.18,0.04]
    legend = ["F-IFNO_constraint", "F-IUFNO_constraint", "IUFNO_constraint", "IFNO_constraint","F-IFNO", "F-IUFNO", "IUFNO", "IFNO","DSM","fDNS"]
    legend1 = [r"$\mathdefault{\Delta T=0.02\tau}$", r"$\mathdefault{\Delta T=0.04\tau}$",r"$\mathdefault{\Delta T=0.1\tau}$",r"$\mathdefault{\Delta T=0.2\tau}$",r"$\mathdefault{\Delta T=0.3\tau}$",r"$\mathdefault{\Delta T=0.4\tau}$"]
