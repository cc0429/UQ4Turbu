...
for k, case_number in enumerate(case_number_list):
    ...
    #-------------------------------------------------------------读入数据，
        ...
        # X 轴设置
        ax.xaxis.set_major_locator(mpl.ticker.MultipleLocator(0.06))
        ...
        #plt.title(r"Errorbar for Ek", fontsize=35, color='black', loc='center', pad=15)
        if vel_k == 3:
            ax.set_ylim(-0.35, 0.08)
        elif vel_k == 5:
            ax.set_ylim(-0.25, 0.05)
        elif vel_k == 7:
            ax.set_ylim(-0.17, 0.013)
            
           
        
        lgd = plt.legend(
            loc='lower left',    bbox_to_anchor=(0.04, 0),   # 自动选择最优位置
            fontsize=35,
            frameon=True,
            framealpha=0,
            edgecolor='black',
